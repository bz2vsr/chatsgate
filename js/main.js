...
function resetToGlobalView() {
    ...
    // Show full user table again and reset its height
    const userStatsCard = document.querySelector('.user-stats-card');
    userStatsCard.style.display = 'block';
    userStatsCard.classList.remove('user-mode-compact');
    
    // Destroy and reinitialize the user table with all users
    if (userTable) {
        userTable.destroy();
        userTable = null;
    }
...
function updateUserTableForUserMode() {
    ...
    // Make the user stats card compact for single user view
    const userStatsCard = document.querySelector('.user-stats-card');
    userStatsCard.classList.add('user-mode-compact');
...
function updateWordPhraseTableForUser() {
    ...
    // Update header to show it's user-specific
    updateWordPhraseHeader();
}

function updateWordPhraseHeader() {
    const header = document.querySelector('#word-phrase-stats-card .card-header h6');
    if (!header) return;
